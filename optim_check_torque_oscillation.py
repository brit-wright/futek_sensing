...
def format_torque(t_list, torque_offset):
    ...
    index = 0
    errors = []
    with open('optim_1600_vib_torque_file.txt', 'w', newline = '') as f:
        for entry in t_list:
            # data processing
    ...
    with open('optim_1600_vib_torque_file.txt', 'r') as f:
        ...
        for line in f:
            ...
            if line != '+\n' and line != ' \n' and line!= '-    \n' and line!= '   \n' and line != '-\n' and line != '-  \n' and line != '    \n' and line != '  \n':
                floated = (float(line) - torque_offset) * (0.9980242908389914 * 1.0159514656155124 * 1.0061498743216253 * 0.994759255032 * 0.9992285428459807) *(1.0044194447822277*1.0058888387503069*0.9963883760432045*0.992546236350996*0.9972841255742625) * 1.0051329185997395 * 0.9971789475175031
...
def format_torque_time(start, times, torque_errors):
    ...
    with open('optim_1600_vib_torque_time_file.txt', 'w') as t:
...
def generate_csv_torque(start, times, torques):
    with open('optim_1600_vibr_torque'+start.strftime("%m_%d_%Y__%H_%M_%S") +'_2000_freq.csv', 'w', newline = '') as csvfile:
...
while time.time() - start <= T:
    ...
    if current_time >= next_sample_time:
        ...
        data = torque_ser.read_until().decode().strip('in-lb\r\n')
        if data:
            torque_list.append(data)
    time.sleep(max(0, next_sample_time - time.time()))  # Sleep until the next sample time
